...
- [Building Bitcoin Core](#building-detkcoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-detkcoin-host/README.md).
...
Clone the git repositories for detkcoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/detkcoin/detkcoin
...
in the detkcoin repository under 'Fetch and create inputs' to install sources which require
...
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the detkcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/detkcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/detkcoin/detkcoin
...
For example:
```bash
URL=https://github.com/laanwj/detkcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit detkcoin=${COMMIT} --url detkcoin=${URL} ../detkcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit detkcoin=${COMMIT} --url detkcoin=${URL} ../detkcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit detkcoin=${COMMIT} --url detkcoin=${URL} ../detkcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the detkcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../detkcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/detkcoin
SIGPATH=/some/root/path/detkcoin-detached-sigs

./bin/gbuild --url detkcoin=${BTCPATH},signature=${SIGPATH} ../detkcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/detkcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/detkcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/detkcoin-osx-build.assert
```
...
[detkcoin-core/gitian.sigs](https://github.com/detkcoin-core/gitian.sigs/) repository, or if that's not possible create a pull
