...
reference the release tarball available from https://detkcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Bitcoin git repository](https://github.com/detkcoin/detkcoin) but are not part of
...
Currently only `Source10` is used, for the example `detkcoin.conf` file.
...
in the detkcoin git repository. At present only an SVG version of the Bitcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the detkcoin
...
The `detkcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `detkcoin-server` package. At this point, most RPM based
...
The `detkcoin-server` package should have SELinux support. How to properly do
...
[Ringing Liberty](https://www.ringingliberty.com/detkcoin/). He has been
...
4. I renamed the `detkcoin` package that contains the Qt GUI to `detkcoin-core` as
that appears to be how the general population refers to it, in contrast to
`detkcoin-xt` or `detkcoin-classic`. I wanted to make sure the general population
